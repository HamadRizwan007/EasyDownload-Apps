            ...
            <div class="row margin_top_bottom">
                ...
                <div class=" col-md-9">
                    ...
                    <div class="py-3 bg-body shadow-sm row app_section mx-2 px-4">
                        ...
                        if(isset($_GET['s']))
                        {
                            ...
                            // {  
                            //     $sql = "SELECT * FROM app_links_new WHERE main_category = '$filtervalues' LIMIT $offset, $no_of_records_per_page";
                            //     $total_pages_sql = "SELECT COUNT(*) FROM app_links WHERE main_category = '$filtervalues'";
                            // }
                            // else
                            // {
                                // $sql = "SELECT * FROM app_links_new WHERE sub_category = '$filtervalues' LIMIT $offset, $no_of_records_per_page";
                                // $total_pages_sql = "SELECT COUNT(*) FROM app_links_new WHERE sub_category = '$filtervalues'";

                                // $sql = "SELECT * FROM app_links_new WHERE main_category = 'Games' LIMIT $offset, $no_of_records_per_page";
                                // $total_pages_sql = "SELECT COUNT(*) FROM app_links_new WHERE main_category = 'Games'";

                            // }
                            // $res = mysqli_query($conn,$query);

                            // echo strlen($filtervalues);
                            if(strlen($filtervalues) == 1)
                            {
                                $sql = "SELECT * FROM app_links_new WHERE CONCAT(name,'_',main_category,sub_category) LIKE '$filtervalues%' LIMIT $offset, $no_of_records_per_page";
                                $total_pages_sql = "SELECT COUNT(*) FROM app_links_new WHERE CONCAT(name,'_',main_category,sub_category) LIKE '$filtervalues%'";
                            }
                            else
                            {
                                $sql = "SELECT * FROM app_links_new WHERE CONCAT(name,'_',main_category,sub_category) LIKE '%$filtervalues%' LIMIT $offset, $no_of_records_per_page";
                        ...
                        // if(true)
                        {
                            while($item = mysqli_fetch_array($res_data)){
                                // for($i=0; $i<16;$i++){
                                if($app_count == 0) //check 6 apps in a col
                                {
                        ...
                        <div class="col-lg-4 px-0">
                            ...
                            <!-- App -->
                            <div class="app ps-2">
                                <a href="single_page.php?slug=<?php echo $item['slug']?>">
                        ...
                        <h3 class="title_category mb-0">No Results Found</h3>
                        </div>
                    ...
                    <center>
                        ...
                        <ul class="pagination mt-3">
                            <li class="<?php if($pageno <= 1){ echo 'disabled'; } ?>"><a
                                    href="<?php echo "?s=".($_GET['s'])?>">
                                    <<</a> </li> <li class="<?php if($pageno <= 1){ echo 'disabled'; } ?>">
                                        <a
                                            href="<?php if($pageno == 2){ echo "?s=".($_GET['s']); } else { echo "?s=".($_GET['s'])."&p=".($pageno - 1); } ?>">PREV</a>
                            </li>


                            <li class="<?php if($pageno >= $total_pages){ echo 'disabled'; } ?>">
                                <a
                                    href="<?php if($pageno >= $total_pages){ echo '?s='.($_GET['s']).'&p=&#'; } else { echo '?s='.($_GET['s']).'&p='.($pageno + 1); } ?>">
                                    NEXT
                                </a>
                            </li>
                            <li class="<?php if($pageno >= $total_pages){ echo 'disabled'; } ?>"><a
                                    href="<?php echo '?s='.($_GET['s']).'&p='.$total_pages; ?>">>></a></li>
                ...
                <div class="col">
                    ...
                    for($i=0; $i<7; $i++)
                    {
                        do{
                            $rand = rand(1,304);
                            $query = "SELECT main_category FROM app_links_new WHERE id = '$rand'";
                            $res = mysqli_query($conn,$query);
                            $item = $res->fetch_assoc();
                            $check = $item['main_category'];
                        }while ($check == 'Kids');
    
                        $query = "SELECT * FROM app_links_new WHERE id = '$rand'" ;
                        $res = mysqli_query($conn,$query);
                        $item = $res->fetch_assoc(); 
                    ?>
        ...
        <div class="col-md-4 mt-1">
            <span class="mx-3"><a class="bar_li white_text" href="../">Home</a></span> <span
                class="green_text_no_hover bar_li">|</span> <span class="mx-3"><a class="bar_li white_text"
                    href="privacy_policy.php">Privacy Policy</a></span> <span class="green_text_no_hover bar_li">|</span> <span
                class="mx-3"><a class="bar_li white_text" href="contact_us.php">Contact Us</a></span>
        </div>
